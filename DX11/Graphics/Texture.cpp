...
Texture::Texture(ID3D11Device* device, const std::string& filePath, aiTextureType type)
{
	...
	if (StringHelper::GetFileExtension(filePath) == ".dds")
	{
		...
		if (FAILED(hr))
		{
			OutputDebugStringA("Failed to Create DDS Texture from File, Will instead use Unloaded Texture Color");
	...
	else
	{
		...
		if (FAILED(hr))
		{
			OutputDebugStringA("Failed to Create WIC Texture from File, Will instead use Unloaded Texture Color");
			this->Initialize1x1ColorTexture(device, Colors::UnloadedTextureColor, type);
		}
		return;
	}
...
Texture::Texture(ID3D11Device* device, const uint8_t* pData, size_t size, aiTextureType type)
{
	...
	if (FAILED(hr))
	{
		OutputDebugStringA("Failed to create Texture from memory.");
	}
}
...
void Texture::InitializeColorTexture(ID3D11Device* device, const Color* colorData, UINT width, UINT height, aiTextureType type)
{
	...
	if (FAILED(hr))
	{
		OutputDebugStringA("Failed to initialize texture from color data.");
	}
	...
	if (FAILED(hr))
	{
		OutputDebugStringA("Failed to create shader resource view from texture generated from color data.");
	} 
}